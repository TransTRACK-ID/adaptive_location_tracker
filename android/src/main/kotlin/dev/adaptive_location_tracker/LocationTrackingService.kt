...
class LocationTrackingService : Service() {
    ...
    private fun httpSend(
        ...
        val uri = Uri.parse(baseUrl).buildUpon()
            ...
            .appendQueryParameter(fieldKey("latitude", "lat"), "%.7f".format(Locale.US, location.latitude))
            .appendQueryParameter(fieldKey("longitude", "lon"), "%.7f".format(Locale.US, location.longitude))
            .appendQueryParameter(fieldKey("speed", "speed"), "%.2f".format(Locale.US, location.speed * 3.6))
            .appendQueryParameter(
                fieldKey("bearing", "bearing"),
                "%.1f".format(Locale.US, if (location.hasBearing()) location.bearing else 0f),
            )
            .appendQueryParameter(fieldKey("altitude", "altitude"), "%.1f".format(Locale.US, location.altitude))
            .appendQueryParameter(fieldKey("accuracy", "accuracy"), "%.1f".format(Locale.US, location.accuracy))
    ...
    private fun httpSendEntry(baseUrl: String, subjectId: String, entry: LocationEntry): Int {
        val uri = Uri.parse(baseUrl).buildUpon()
            .appendQueryParameter(fieldKey("subjectId", "id"), subjectId)
            .appendQueryParameter(fieldKey("timestamp", "timestamp"), "%.0f".format(Locale.US, entry.timestampSec))
            .appendQueryParameter(fieldKey("latitude", "lat"), "%.7f".format(Locale.US, entry.lat))
            .appendQueryParameter(fieldKey("longitude", "lon"), "%.7f".format(Locale.US, entry.lon))
            .appendQueryParameter(fieldKey("speed", "speed"), "%.2f".format(Locale.US, entry.speed * 3.6))
            .appendQueryParameter(fieldKey("bearing", "bearing"), "%.1f".format(Locale.US, entry.heading))
            .appendQueryParameter(fieldKey("altitude", "altitude"), "%.1f".format(Locale.US, entry.altitude))
            .appendQueryParameter(fieldKey("accuracy", "accuracy"), "%.1f".format(Locale.US, entry.accuracy))
